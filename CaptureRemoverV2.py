...
def doCRV2():
    ...
    time.sleep(1)
    print("---------------------------------------------------------------")
    separator = input(str("How are the captures separated? "))
    time.sleep(1)
    print("---------------------------------------------------------------")
    createOut = open("Capture Remover-" + combo,"w+")
    with open(combo) as infile:
        for line in infile:
            with open("Capture Remover-" + combo, "a") as out:
                accountSplitted = line.split(separator)
                account = accountSplitted[0].replace(" ", "")
                out.write(account + "\n")
    print("Done!")
    doWTD()